...
/**
 * An abstraction for a Tetris board, which allows for querying its state and
 * applying actions.
 */
public interface Board {
    ...
    enum Action {
         ...
         * intersect with the stack, the piece should instead be placed at its current position.
         */
        DOWN,
    ...
    /**
     * Applies the given action to the board, mutating its state.
     */
     ...
     * of the board, where Y increases upwards and X increases to the right. Due to SRS
     ...
     *
     * If the piece is placed in a position that would cause it to intersect with existing pieces
    ...
    /**
     * Returns the max column height present in the board (as computed by {@code getColumnHeight}).
     ...
     *
     * Note that, in the board coordinate system, (0, 0) is the lower-left hand corner
     * of the board, where Y increases upwards and X increases to the right.
     */
